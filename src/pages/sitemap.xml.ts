...
export const GET: APIRoute = () => {
  const paths = ["/", "/products", "/work", "/research", "/engineering", "/builds", "/brand", "/about", "/contact", ...products.map((product) => `/products/${product.slug}`), ...caseStudies.map((study) => `/case-studies/${study.slug}`)];
